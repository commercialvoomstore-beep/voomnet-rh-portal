...
import { Employee, ChatMessage, AbsenceRequest, StatutContrat } from '@/data/mockData';
...
export const fetchNeonEmployees = async (): Promise<Employee[] | null> => {
  return executeNeonQuery(async (sql) => {
    ...
    return rows.map((r: any) => ({
      ...
      prenom: r.first_name,
      email: r.email,
      telephone3CX: r.phone || r.matricule || '',
      role: r.role === 'SUPERADMIN' ? 'SuperAdmin' : r.role === 'ADMIN' ? 'Admin RH' : 'Employé',
      poste: r.position,
      departement: r.department,
      statut: (r.status as StatutContrat) || 'CDI',
      soldeConges: 25,
      salaireBase: Number(r.base_salary) || 350000,
      dateEmbauche: r.hire_date || '2023-01-01',
