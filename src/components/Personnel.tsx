...
export const Personnel: React.FC = () => {
  ...
  const getStatutBadge = (statut: StatutContrat) => {
    switch (statut) {
      case 'STAGIAIRE':
        return (
          <span className="px-2.5 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-200 text-[11px] font-bold flex items-center gap-1 w-max">
      ...
      case 'CDD':
        return (
          <span className="px-2.5 py-1 rounded-full bg-cyan-50 text-cyan-700 border border-cyan-200 text-[11px] font-bold w-max">
            CDD
          </span>
        );
      case 'CDI':
  ...
  const getRoleBadge = (role: RoleType) => {
    switch (role) {
      case 'SuperAdmin':
        return (
          <span className="px-2.5 py-0.5 rounded text-[10px] font-extrabold bg-purple-100 text-purple-800 border border-purple-300">
            SuperAdmin
          </span>
        );
      case 'Admin':
        return (
          <span className="px-2.5 py-0.5 rounded text-[10px] font-extrabold bg-sky-100 text-sky-800 border border-sky-300">
            Admin
          </span>
        );
      case 'Employé':
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-xl border border-slate-200 text-xs font-semibold text-slate-600">
            <span className="px-2 text-[10px] uppercase font-bold text-slate-400">Contrat :</span>
          ...
          {/* Role Filter */}
          <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-xl border border-slate-200 text-xs font-semibold text-slate-600">
            <span className="px-2 text-[10px] uppercase font-bold text-slate-400">Rôle :</span>
              ...
              {filteredEmployees.map((emp) => (
                <tr key={emp.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="py-3.5 px-4 font-mono">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 border border-slate-200 font-bold text-blue-600">
                  ...
                  <td className="py-3.5 px-4">
                    <div className="font-semibold text-slate-900">{emp.poste}</div>
                    <div className="text-[11px] text-slate-500 flex items-center gap-1">
                  ...
                  <td className="py-3.5 px-4 font-mono font-bold text-slate-800">
                    <div className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-200 w-max text-xs">
                  ...
                  <td className="py-3.5 px-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="p-1.5 bg-slate-100 hover:bg-slate-200 text-blue-600 rounded-lg border border-slate-300 transition-all"
                      >
          ...
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto text-slate-900">
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-purple-300 rounded-xl text-purple-900 text-xs font-bold"
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">Photo de Profil (URL)</label>
                  <input
                    type="text"
      ...
      {/* Edit Modal */}
      {editingEmployee && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 text-slate-900">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-lg shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto">
            <button
            ...
            <p className="text-xs text-slate-500 mb-4">
              Matricule : <strong className="text-blue-600 font-mono">{editingEmployee.matricule}</strong>
            </p>
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center gap-4">
                ...
                <div className="flex-1 space-y-1.5">
                  <label className="block text-[11px] font-bold text-slate-700">URL Photo de Profil</label>
                  <input
                    type="text"
                    value={editFormState.avatar || ''}
                    onChange={(e) => setEditFormState({ ...editFormState, avatar: e.target.value })}
                    className="w-full px-2.5 py-1.5 bg-white border border-slate-300 rounded-lg text-slate-900 text-xs font-mono"
                  />
                  <button
                    type="button"
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-2 py-1.5 bg-white border border-slate-300 rounded-xl text-slate-900 text-xs font-bold"
                    >
                      <option value="Employé">Employé</option>
                      <option value="Admin">Admin</option>
                      <option value="SuperAdmin">SuperAdmin</option>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Notes Administratives RH</label>
                <textarea
                  rows={2}
